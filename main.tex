...
\textbf{Communication efficient online learning.} Suppose we want to conduct online learning on multiple nodes. Those nodes are connected in a decentralized way. At every iteration, the local model has to be broadcast to its neighbours. During online learning, the communication efficiency also needs to be considered. In the case, $g_{i,t}(\x_{i,t})$ represents the loss incurred by the learning model, and $h_{t}(\x_{i,t};\xi_{i,t})$ represents the loss incurred by some a random method, e.g., random quantization method \citep{dd}, to guarantee the communication efficiency. 
 
\textbf{Online learning with privacy.} Suppose we want to conduct online learning on a decentralized network. But, there is a hacker who can sniff at the network, and obtains the transmitted data packages. To protect the privacy, we use a randomization method to handle the local model before transmitting it in the network. In the case, $g_{i,t}(\x_{i,t})$ represents the loss incurred by the learning model. $h_{t}(\x_{i,t};\xi_{i,t})$ represents the loss incurred by some a randomization encryption method, e.g., parameter perturbation \citep{dd}, to protect the privacy. 

%\textbf{Online music recommendation.} Suppose we want to decide whether to recommend a music to Bob by using a public dataset consisting of historical browser records on Youtube. But, some values of features in those records are not reliable. For example, Alice's browser record is in the public dataset. But Alice does not want to let others know her real birthday and age. She submits random numbers for such information when signing up as an Youtube user. \textit{Note that those unreliable values, e.g., Alice's age and birthday, usually do not change, which is modeled by an unknown distribution. But, other reliable values, e.g., Alice's perference to a music, may change over time, which is a classic setting for an online learning problem. }  
%
%Suppose we use logistic regression to conduct online music recommendation. Generally, suppose a users' behavior record is denoted by $\a\in\RR^{d\times 1}$.  We denote the label by $\y\in \RR$. If a user listens to the music, then the label is $\y = 1$, otherwise $\y = -1$. Without loss of generality, features corresponding to those unreliable values are denoted by the beginning $s$ features.  At the $t$-th iteration, the instance is $\a_t$, and the label is $\y_t$. Denote $g_{t}(\x) = \log \lrincir{1 + \exp\lrincir{-\y_{t}\a\Tr_{t} \hat{\I} \x}}$, where $\hat{\I}$ is yielded by letting the first $s$ diagonal elements of identity matrix be $0$s. In the case, $\xi_{t} = \check{\I} \a_t$, and $\xi_{t} \sim D$ for any $t\in[T]$. $h_t(\x; \xi_{t}) = \log \lrincir{1 + \exp\lrincir{-\y_{t}\xi_{t}\Tr \x}}$, where $\check{\I}$ is yielded by letting the last $d-s$ diagonal elements of identity matrix be $0$s. 
%
%
