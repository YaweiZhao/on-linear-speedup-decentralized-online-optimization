...
We generate a data matrix $\A$ with $\A=0.1\tilde{\A}+0.9\hat{\A}$, where $\tilde{\A}$ represents the adversary part of data, and $\hat{\A}$ represents the stochastic part of data. $\tilde{\A}_{i,t}$ is the instance on the $i$-th node at the $t$-th iteration, and $\y_{i,t}\in\{1,-1\}$ is its label. The dimension of every instance is $d = 10$. Specifically, $\A$ and $\y$ are generated as follows.
\begin{enumerate}
\item $\tilde{\A}$ is a random matrix, and elements of every column of $\tilde{\A}$ is sampled from the interval $[-0.5,0.5]$ randomly.
\item $\y_{i,t}\in\{1,-1\}$ is generated randomly. When $\y_{i,t}=1$, $\hat{\A}_{i,t}$ is generated by sampling from a multi-variate Normal distribution $\hat{\A}_{i,t} \sim N((1+0.5\sin(t))\cdot\1, \I)$. When $\y_{i,t} = -1$, $\hat{\A}_{i,t}$ is generated by sampling from another multi-variate Normal distribution $\hat{\A}_{i,t} \sim N((-1+0.5\sin(t))\cdot\1, \I)$.
\end{enumerate} Thus, we let $f_{i,t}(\x;\xi_{i,t}) = \log\lrincir{1+\exp(-\y_{i,t}\A_{i,t}\Tr \x)} + \frac{\gamma}{2}\lrnorm{\x}^2$, where $\gamma = 10^{-3}$ is the given hyper-parameter. The learning rate $\eta$ is set to be $\sqrt{\frac{M}{T}}$. We evaluate the performance of our decentralized online gradient method, i.e., DOG with that of the centralized online gradient method (COG).   



First, we simulate a decentralized network consisting of $100$ nodes. Those nodes are connected by using a ring topology. We test the averaged loss and the averaged cumulative loss for every node to compare the performance of DOG and COG.  As shown in Figure \ref{figure_synthetic_loss}, both DOG and COG is effective to optimize the decision variable to follow the dynamics of the environment. COG has slightly better performance than DOG, and the superiority is unsignificant, especially for a small $M$. 

\begin{figure*}[!h]
\setlength{\abovecaptionskip}{0pt}
\setlength{\belowcaptionskip}{0pt}
\centering 
\subfigure[\textit{Comparison of the averaged loss}]{\includegraphics[width=0.45\columnwidth]{figure_decen_cen_ave_loss_iterations}\label{figure_decen_cen_ave_loss_iterations}}
\subfigure[\textit{Comparison of the averaged cumulative loss}]{\includegraphics[width=0.45\columnwidth]{figure_decen_cen_ave_cumu_loss_iterations}\label{figure_decen_cen_ave_cumu_loss_iterations}}
\caption{The performance of DOG is comparable to that of COG.}
\label{figure_synthetic_loss}
\end{figure*}


Second, we evaluate the performance of DOG by varying the network size. As shown in Figure \ref{figure_decen_cen_ave_cumu_loss_nodes}, the averaged cumulative loss decreases effectively for a large network. But, when the size of the network is large than a threshold, the performance does not have significant improve.  The reason is due to the adversary data, which impedes to improve the decison variable for a large decentralized network. It matches with our theoretical result.  After that, we change the topology of the network, and still evaluate the performance of DOG. The topology  \textit{Fully connected} means all nodes are connected, where DOG de-generates to be COG. The topology  \textit{No connected} means any node does not connect with others. 
The topology  \textit{WattsStrogatz} represents a Watts-Strogatz small-world graph. There is a parameter can be tuned, e.g., $0.5$ or $1$ in legend of Figures \ref{figure_decen_cen_ave_loss_iterations_net_types} and \ref{figure_decen_cen_cumu_loss_iterations_net_type}, to control the number of random edges in the Watts-Strogatz small-world graph.  As illustrated in Figures \ref{figure_decen_cen_ave_loss_iterations_net_types} and \ref{figure_decen_cen_cumu_loss_iterations_net_type}, when the topology \textit{No connected} leads to a worse performance than others, and the topology  \textit{Fully connected} has the best performance. But, there is no significant difference between all topologies except  \textit{No connected}.
...
\setlength{\belowcaptionskip}{0pt}
\centering 
\subfigure[\textit{Averaged cumulative loss v.s. Network size}]{\includegraphics[width=0.33\columnwidth]{figure_decen_cen_ave_cumu_loss_nodes}\label{figure_decen_cen_ave_cumu_loss_nodes}}
\subfigure[\textit{Averaged loss v.s. Network types}]{\includegraphics[width=0.32\columnwidth]{figure_decen_cen_ave_loss_iterations_net_types}\label{figure_decen_cen_ave_loss_iterations_net_types}}
\subfigure[\textit{Averaged cumulative loss v.s. Network types}]{\includegraphics[width=0.33\columnwidth]{figure_decen_cen_cumu_loss_iterations_net_type}\label{figure_decen_cen_cumu_loss_iterations_net_type}}
\caption{Comparison of the performance of DOG by varying the network size, and the topology of the network.}
\label{figure_synthetic_loss_net_types}
\end{figure*}
...
\end{itemize}
All values of a feature have been normalized to be zero mean and one variance. The budget of dynamics, namely $M$ is set to be $10$. The learning rate, namely $\eta$ in Algorithm \ref{algo_DOG} is set to be $\sqrt{\frac{5M}{100T}}$.  
