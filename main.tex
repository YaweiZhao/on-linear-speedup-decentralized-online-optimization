...
We conduct online logistic regression in the empirical studies. We let $f_{i,t}(\x;\xi_{i,t}) = \log\lrincir{1+\exp(-\y_{i,t}\A_{i,t}\Tr \x)} + \frac{\gamma}{2}\lrnorm{\x}^2$, where $\gamma = 10^{-3}$ is the given hyper-parameter. The learning rate $\eta$ is set to be $\sqrt{\frac{M}{100T}}$.  
...
\textbf{Synthetic data} We generate a data matrix $\A=\A_1 + \A_2+ \cdots + \A_n$, where $\A_i$ is placed on the $i$-th node, and $\A_i=0.1\tilde{\A}_i+0.9\hat{\A}_i$, where $\tilde{\A}_i$ represents the adversary part of data, and $\hat{\A}_i$ represents the stochastic part of data. $\y_{i}\in\{1,-1\}$ is the label of an instance $\hat{\A}_{i,t}$. The dimension of every instance is $d = 10$. Specifically,  elements of $\tilde{\A}_i$ is sampled from the interval $[-0.5+\sin(i),0.5+\sin(i)]$ randomly. Note that $\tilde{\A}_i$ and $\tilde{\A}_j$ with $i\neq j$ are different distributions. Besides, $\y_{i,t}\in\{1,-1\}$ is generated randomly. When $\y_{i,t}=1$, $\hat{\A}_{i,t}$ is generated by sampling from a multi-variate Normal distribution $\hat{\A}_{i,t} \sim N((1+0.5\sin(t))\cdot\1, \I)$. When $\y_{i,t} = -1$, $\hat{\A}_{i,t}$ is generated by sampling from another multi-variate Normal distribution $\hat{\A}_{i,t} \sim N((-1+0.5\sin(t))\cdot\1, \I)$. As illustrated in Figure \ref{figure_illus_dynamics}, the data distribution changes over time, and the optimal learning model thus change over time, where the dynamic regret is practical and necessary to measure the goodness of a learning model. 

\textbf{Real data} We use three real time-series datasets: \textit{room-occupancy}\footnote{\url{https://archive.ics.uci.edu/ml/datasets/Occupancy+Detection+}},  \textit{online-retail}\footnote{\url{https://archive.ics.uci.edu/ml/datasets/Online+Retail}}, and \textit{spam}\footnote{\url{http://mlkd.csd.auth.gr/concept_drift.html}}.  The data distribution of those datasets changes over time in those practial scenarios, leading to the change of the optimal learning model during online learing. In those dynamic environment, the dynamic regret is practial and necessary. 
...
The average loss $\frac{1}{nT}\sum_{i=1}^n\sum_{t=1}^T f_{i,t;\xi_{i,t}}(\x_{i,t})$ is used as a metric.  We use it to varify: 
\begin{enumerate}
\item whether Decentralized Online Gradient method (DOG), i.e., Algorithm \ref{algo_DOG}, has comparable performance with the Centralized Online Gradient method (COG).
\item whether DOG has a good performance with the increase of the network size.
\item whether DOG has a good performance with the change of the network topology.
\end{enumerate}
...
First, we simulate a decentralized network consisting of $100$ nodes to handle the synthetic data, and a network consisting $5$ nodes to handle the real data. Those nodes are connected by using a ring topology.  As shown in Figure \ref{figure_synthetic_loss}, both DOG and COG are effective to optimize the decision variable, and they have very similar performance. Comparing with COG, DOG is good enough to conduct online learning. 
...
\subfigure[\textit{online-retail}, $5$ nodes]{\includegraphics[width=0.48\columnwidth]{figure_decen_cen_ave_loss_iterations_online_retail}\label{figure_decen_cen_ave_loss_iterations_online_retail}}
...
\caption{The performance of DOG is comparable to that of COG.}
...
\setlength{\belowcaptionskip}{0pt}
\centering 
\subfigure[\textit{Network size}]{\includegraphics[width=0.48\columnwidth]{figure_decen_cen_ave_loss_nodes}\label{figure_decen_cen_ave_loss_nodes}}
\subfigure[\textit{Network topology}]{\includegraphics[width=0.48\columnwidth]{figure_decen_cen_ave_loss_iterations_net_types}\label{figure_decen_cen_ave_loss_iterations_net_types}}
\caption{The performance of DOG is comparable to that of COG.}
\label{figure_synthetic_loss}
\end{figure*}


Second, we evaluate the performance of DOG by varying the network size. As shown in Figure \ref{figure_decen_cen_ave_loss_nodes}, the average loss decreases effectively for a large network. After that, we change the topology of the network, and still evaluate the performance of DOG. The topology  \textit{Fully connected} means all nodes are connected, where DOG de-generates to be COG. The topology  \textit{WattsStrogatz} represents a Watts-Strogatz small-world graph. There is a parameter can be tuned, e.g., $0.5$ or $1$ in the legend of Figure \ref{figure_decen_cen_ave_loss_iterations_net_types}, to control the number of random edges in the Watts-Strogatz small-world graph.  As illustrated in Figures \ref{figure_decen_cen_ave_loss_iterations_net_types}, the topology  \textit{Fully connected} has the best performance. But, there is no significant difference between all topologies.
...
\nonumber
& \EE_{ \Xi_{n,T} \sim \Dcal_{T} } \sum_{t=1}^T\sum_{i=1}^n f_{i,t}(\x_{i,t};\xi_{i,t}) - f_t(\x_t^\ast;\xi_{i,t}) \\ \nonumber
\le & \eta T n\beta \lrincir{1 + \frac{1}{2(1-\rho)^2}+16\beta} G + (1-\beta)\eta T\sigma^2 + 4n(1-\beta) \lrincir{ \EE_{ \Xi_{n,T} \sim \Dcal_{T} } \sum_{t=1}^T  \lrincir{H_t(\bar{\x}_t) - H_t(\bar{\x}_{t+1})}  } \\ \nonumber
