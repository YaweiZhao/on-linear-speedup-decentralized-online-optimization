...
\begin{align}
\nonumber
\Rcal_T^{DOG} = & \sum_{i=1}^n\sum_{t=1}^T \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }f_{i,t}(\x_{i,t};\xi_{i,t}) - \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }f_{i,t}(\x_t^\ast;\xi),
\end{align} where, for any $\x$,
\begin{align}
\nonumber
f_{i,t}(\x;\xi) := \beta g_{i,t}(\x) + (1-\beta) h_t(\x; \xi)
...
\begin{itemize}
\item For any $i\in[n]$ and $t\in[T]$, we assume $\max\left\{\lrnorm{\nabla g_{i,t}(\x)}^2, \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{\nabla h_t(\x;\xi_{i,t})}^2, \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{\nabla f_{i,t}(\x; \xi_{i,t})}^2 \right\} \le G$, and $\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrnorm{\nabla h_t(\x; \xi_{i,t}) - \nabla h_t(\x)}^2 \le \sigma_{i,t}^2$.
\item For any $\x$ and $\y$, we assume $\lrnorm{\x-\y}^2 \le R$.
\item For any $i\in[n]$ and $t\in[T]$, we assume the function $f_{i,t}$ is convex and differentiable, and the function $h_t$ has  $L$ Lipichitz gradients.
\end{itemize}
...
Use Assumptions \ref{assumption_bounded_gradient_domain}, and choose $\eta>0$ in Algorithm \ref{algo_dog}.  we have
\begin{align}
\nonumber
& \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \sum_{t=1}^T\sum_{i=1}^n f_{i,t}(\x_{i,t};\xi_{i,t}) - f_t(\x_t^\ast;\xi_{i,t}) \\ \nonumber
\le & \eta nTG\lrincir{ 21\beta + \frac{6\eta}{(1-\rho)^2}\lrincir{ L + \frac{\eta L^2}{\beta} + 6\eta L^2 + \frac{\beta}{\eta} } + 3L\eta\lrincir{\frac{1}{\beta} + 4}  } + \lrincir{\frac{1}{\beta}+4}n \lrincir{h_t(\bar{\x}_1) - h_t(\bar{\x}_{T+1})} \\ \nonumber
& + \lrincir{ \lrincir{ L + \frac{\eta L^2}{\beta}  + 6\eta L^2  + \frac{\beta}{\eta} } \frac{4\eta^2}{(1-\rho)^2} + \frac{\nu(\eta + 8nL\eta^2) + 2nL\eta^2}{n\beta} } \sum_{t=1}^T\sum_{i=1}^n \sigma_{i,t}^2 + \frac{n}{2\eta}\lrincir{ 4\sqrt{R}D + R  }.
\end{align}  
...
\begin{align}
\nonumber
\Rcal_T^{DOG} \le \Ocal{\sqrt{ (1+n\beta)nDT } }.
\end{align}
...
\begin{align}
\nonumber
& \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \frac{1}{n}\sum_{i=1}^n f_{i,t}(\x_{i,t};\xi_{i,t}) - f_t(\x_t^\ast;\xi_{i,t}) \\ \nonumber
=& \frac{1}{n}\sum_{i=1}^n \beta \lrincir{g_{i,t}(\x_{i,t}) - g_{i,t}(\x_t^\ast)} + (1-\beta) \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrincir{h_t(\x_{i,t};\xi_{i,t}) - h_t(\x_t^\ast;\xi_{i,t})} \\ \nonumber
\le & \frac{1}{n}\sum_{i=1}^n \beta \lrangle{\nabla g_{i,t}(\x_{i,t}), \x_{i,t} - \x_t^\ast } + (1-\beta) \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrangle{\nabla h_t(\x_{i,t};\xi_{i,t}), \x_{i,t} - \x_t^\ast } \\ \nonumber
 ...
 & + \frac{1}{n}\sum_{i=1}^n (1-\beta) \lrincir{ \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrangle{\nabla h_t(\x_{i,t};\xi_{i,t}), \x_{i,t} - \bar{\x}_t } +  \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrangle{\nabla h_t(\x_{i,t};\xi_{i,t}), \bar{\x}_t - \bar{\x}_{t+1} } } \\ \nonumber 
...
\refabovecir{\le}{\textcircled{1}} & \frac{L}{n}\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t}^2 + \frac{1}{n}\sum_{i=1}^n\lrangle{\nabla h_t(\bar{\x}_t), \x_{i,t} - \bar{\x}_t } \\ \nonumber
...
\le & \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrincir{h_t(\bar{\x}_t) - h_t(\bar{\x}_{t+1})} + 4G\eta\beta^2 +  \frac{\eta L^2(1-\beta)^2}{n}\sum_{i=1}^n\lrnorm{\bar{\x}_t - \x_{i,t} }^2 + 3GL\eta^2  + \frac{2L\eta^2}{n}\sum_{i=1}^n \sigma_{i,t}^2.
\end{align} Substituting \eqref{equa_theorem_temp1} into \eqref{equa_theorem_temp0}, we obtain
\begin{align}
\nonumber
& J_1(t) \\ \nonumber 
\le & \frac{L}{n}\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t}^2 + \lrincir{ \frac{1}{\nu} \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrincir{h_t(\bar{\x}_t) - h_t(\bar{\x}_{t+1})} + \frac{4G\eta\beta^2}{\nu} +  \frac{\eta L^2(1-\beta)^2}{n\nu}\sum_{i=1}^n\lrnorm{\bar{\x}_t - \x_{i,t} }^2 } \\ \nonumber 
&+ \frac{3GL \eta^2}{\nu} + \frac{2L\eta^2}{n\nu} \sum_{i=1}^n \sigma_{i,t}^2  + \frac{\nu}{2n\eta}\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2  \\ \nonumber
= & \lrincir{ \frac{L}{n} + \frac{\eta L^2(1-\beta)^2}{n\nu} + \frac{\nu}{2n\eta} }\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2 + \frac{1}{\nu} \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrincir{h_t(\bar{\x}_t) - h_t(\bar{\x}_{t+1})} \\ \nonumber 
...
\begin{align}
\nonumber
J_2(t) = & \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrangle{\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t};\xi_{i,t}), \bar{\x}_t - \bar{\x}_{t+1} } \\ \nonumber
\le & \lrincir{\frac{\eta}{2}\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrnorm{\frac{1}{n}\sum_{i=1}^n \nabla h_t(\x_{i,t};\xi_{i,t})}^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 } \\ \nonumber
\le & \lrincir{\frac{\eta}{2}\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t};\xi_{i,t}) - \nabla h_t(\x_{i,t}) + \nabla h_t(\x_{i,t})}^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 } \\ \nonumber
\le & \lrincir{ \eta\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t};\xi_{i,t}) - \nabla h_t(\x_{i,t})}^2 + \eta \lrnorm{\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t})}^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 } \\ \nonumber
\le & \frac{\eta}{n^2} \sum_{i=1}^n\sigma_{i,t}^2 + \eta \lrnorm{\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t}) - \nabla h_t(\bar{\x}_t) + \nabla h_t(\bar{\x}_t)}^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 \\ \nonumber
\le & \frac{\eta}{n^2} \sum_{i=1}^n\sigma_{i,t}^2 + 2\eta \lrnorm{\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t}) - \nabla h_t(\bar{\x}_t) }^2 + 2\eta \lrnorm{\nabla h_t(\bar{\x}_t)}^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 \\ \nonumber
\le & \frac{\eta}{n^2} \sum_{i=1}^n\sigma_{i,t}^2 + \frac{2\eta L^2}{n}\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t }^2 + 2\eta \lrnorm{\nabla h_t(\bar{\x}_t)}^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2.
\end{align} 
...
\nonumber
& J_2(t) \\ \nonumber 
\le & \frac{\eta}{n^2} \sum_{i=1}^n\sigma_{i,t}^2 + \frac{2\eta L^2}{n}\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t }^2 + 4\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrincir{h_t(\bar{\x}_t) - h_t(\bar{\x}_{t+1})} + 16 G\eta \beta^2 + \frac{4\eta L^2 (1-\beta)^2}{n}\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t}^2  \\ \nonumber
& + 12GL\eta^2 + \frac{8L\eta^2}{n} \sum_{i=1}^n \sigma_{i,t}^2  + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 \\ \nonumber
\le & \frac{\eta + 8nL\eta^2}{n^2} \sum_{i=1}^n\sigma_{i,t}^2 + \lrincir{ 2\eta L^2 + 4\eta L^2(1-\beta)^2 }\frac{1}{n}\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t}^2 + 4\lrincir{ \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_t) - \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_{t+1}) } \\ \nonumber 
& + 16G\eta \beta^2 + 12GL\eta^2 + \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2.
\end{align} Therefore, we obtain
\begin{align}
\nonumber
& I_2(t) \\ \nonumber 
= & (1-\beta)(J_1(t) + J_2(t)) \\ \nonumber
\le &  \lrincir{ \frac{L}{n} + \frac{\eta L^2(1-\beta)^2}{n\nu} + \frac{\nu}{2n\eta} + \frac{6\eta L^2}{n} }\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2  + \lrincir{\frac{1}{\nu} + 4}\lrincir{ \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_t) - \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_{t+1}) }\\ \nonumber 
&+ (1-\beta) \lrincir{ 4G\eta\beta^2\lrincir{\frac{1}{\nu} + 4} +  \frac{1}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 }  + \frac{(1-\beta)3LG\eta^2}{\nu} \lrincir{ \frac{1}{\nu} + 4 }  + (1-\beta)\frac{\nu(\eta + 8nL\eta^2) + 2nL\eta^2}{n^2\nu} \sum_{i=1}^n\sigma_{i,t}^2 \\ \nonumber
\le &  \lrincir{ \frac{L}{n} + \frac{\eta L^2}{n\nu} + \frac{\nu}{2n\eta} + \frac{6\eta L^2}{n} }\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2  + \lrincir{\frac{1}{\nu} + 4}\lrincir{ \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_t) - \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_{t+1}) }\\ \nonumber 
&+  4G\eta\beta^2\lrincir{\frac{1}{\nu} + 4} +  \frac{1-\beta}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2   + 3LG\eta^2 \lrincir{ \frac{1}{\nu} + 4 }  + \frac{\nu(\eta + 8nL\eta^2) + 2nL\eta^2}{n^2\nu} \sum_{i=1}^n\sigma_{i,t}^2.
\end{align}
...
&+  4G\eta\beta^2\lrincir{\frac{1}{\nu} + 4} +  \frac{1-\beta}{2\eta} \lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 + 3LG\eta^2\lrincir{ \frac{1}{\nu} + 4 } + \frac{\nu(\eta + 8nL\eta^2) + 2nL\eta^2}{n^2\nu} \sum_{i=1}^n\sigma_{i,t}^2\\ \nonumber
\le & \lrincir{1 + 4\beta\lrincir{\frac{1}{\nu}+4}}\beta G \eta + \lrincir{ \frac{L}{n} + \frac{\eta L^2}{n\nu} + \frac{\nu}{2n\eta} + \frac{6\eta L^2}{n}  + \frac{\beta}{2n\eta} }\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2 \\ \nonumber 
&+ \lrincir{\frac{1}{\nu} + 4}\lrincir{ \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_t) - \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_{t+1})} + \frac{1}{2\eta}\lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 \\ \nonumber 
...
\le & \lrincir{1 + 4\beta\lrincir{\frac{1}{\nu}+4}}T\beta G \eta + \lrincir{ \frac{L}{n} + \frac{\eta L^2}{n\nu} + \frac{\nu}{2n\eta} + \frac{6\eta L^2}{n}  + \frac{\beta}{2n\eta} }\sum_{t=1}^T\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2 \\ \nonumber 
&+ \lrincir{\frac{1}{\nu} + 4}\lrincir{h_1(\bar{\x}_1) - h_{T+1}(\bar{\x}_{T+1})} + \frac{1}{2\eta}\sum_{t=1}^T\lrnorm{ \bar{\x}_t - \bar{\x}_{t+1}}^2 \\ \nonumber 
...
\begin{align}
\nonumber
& \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1,t=1}^{n,T} } \sum_{t=1}^T\sum_{i=1}^n f_{i,t}(\x_{i,t};\xi_{i,t}) - f_t(\x_t^\ast;\xi_{i,t}) \\ \nonumber
\le & n \sum_{t=1}^T \lrincir{ I_1(t) + I_2(t) + I_3(t) } \\ \nonumber
\le & \lrincir{1 + 4\beta\lrincir{\frac{1}{\nu}+4}}nT\beta G \eta + \lrincir{ L + \frac{\eta L^2}{\nu} + \frac{\nu}{2\eta} + 6\eta L^2  + \frac{\beta}{2\eta} }\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1,t=1}^{n,T} }\sum_{t=1}^T\sum_{i=1}^n\lrnorm{\x_{i,t} - \bar{\x}_t }^2 \\ \nonumber 
...
\begin{align}
\nonumber
& \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }h_t(\bar{\x}_{t+1}) \\ \nonumber
\le & h_t(\bar{\x}_t) + \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrangle{\nabla h_t(\bar{\x}_t), \bar{\x}_{t+1} - \bar{\x}_t} + \frac{L}{2}\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{\bar{\x}_{t+1} - \bar{\x}_t}^2 \\ \nonumber
= & h_t(\bar{\x}_t) + \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrangle{\nabla h_t(\bar{\x}_t), -\frac{\eta}{n}\sum_{i=1}^n \nabla f_{i,t}(\x_{i,t};\xi_{i,t})} + \frac{L}{2} \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{\frac{\eta}{n}\sum_{i=1}^n \nabla f_{i,t}(\x_{i,t};\xi_{i,t})}^2 \\ \nonumber
= & h_t(\bar{\x}_t) + \lrangle{\nabla h_t(\bar{\x}_t), -\frac{\eta}{n}\sum_{i=1}^n \nabla f_{i,t}(\x_{i,t})} + \frac{L\eta^2}{2n}\sum_{i=1}^n\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{ \nabla f_{i,t}(\x_{i,t};\xi_{i,t})}^2 \\ \nonumber
...
& + \frac{L\eta^2}{2n}\sum_{i=1}^n\EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n }\lrnorm{ \nabla f_{i,t}(\x_{i,t};\xi_{i,t})}^2 \\ \nonumber
...
\nonumber
& \frac{\eta}{2}\lrnorm{\nabla h_t(\bar{\x}_t)}^2 + \frac{\eta}{2}\lrnorm{\frac{1}{n}\sum_{i=1}^n \nabla f_{i,t}(\x_{i,t})}^2 \\ \nonumber 
\le & \EE_{ \{\xi_{i,t} \sim D_{i,t}\}_{i=1}^n } \lrincir{h_t(\bar{\x}_t) - h_t(\bar{\x}_{t+1})} + 2G\eta\beta^2 +  \frac{\eta L^2(1-\beta)^2}{n}\sum_{i=1}^n\lrnorm{\bar{\x}_t - \x_{i,t} }^2 + 3GL\eta^2  + \frac{2L\eta^2}{n}\sum_{i=1}^n \sigma_{i,t}^2.
...
\begin{align}
\nonumber
& \EE_{ \{ \xi_{i,t} \sim D_{i,t} \}_{i=1,t=1}^{n,T} } \sum_{i=1}^n\sum_{t=1}^T \lrnorm{\x_{i,t} - \bar{\x}_t}^2  \\ \nonumber 
\le & \EE_{ \{ \xi_{i,t} \sim D_{i,t} \}_{i=1,t=1}^{n,T} } \sum_{t=1}^T \lrincir{ \sum_{s=1}^{t-1} \eta \rho^{t-s-1} \lrnorm{\G_s}_F}^2  \\ \nonumber
\le & \frac{\eta^2}{(1-\rho)^2} \EE_{ \{ \xi_{i,t} \sim D_{i,t} \}_{i=1,t=1}^{n,T} } \lrincir{  \sum_{t=1}^T \lrnorm{\G_t}_F^2 } \\ \nonumber
\le & \frac{\eta^2}{(1-\rho)^2} \lrincir{  \sum_{t=1}^T \sum_{i=1}^n \EE_{ \xi_{i,t} \sim D_{i,t} } \lrnorm{\nabla f_{i,t}(\x_{i,t}; \xi_{i,t})}^2 } \\ \nonumber
\le & \frac{\eta^2}{(1-\rho)^2} \lrincir{  6nTG + 4\sum_{t=1}^T\sum_{i=1}^n \sigma_{i,t}^2 } \\ \nonumber
\le & \frac{\eta^2}{(1-\rho)^2}   \lrincir{ 6nTG + 4\sum_{t=1}^T\sum_{i=1}^n \sigma_{i,t}^2 }.
\end{align}
It completes the proof.
