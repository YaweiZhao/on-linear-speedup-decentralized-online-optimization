...
\end{enumerate}    Then, we conduct decentralized online logistic regression by using the synthetic data.   At every iteration $t$, the reference point $\x_t^\ast$ is approximately computed by performing  gradient descent $5$ iterations. The dynamics budget $M$ is $43436$ for PDR, $39106$ for ODR ($\beta=0.9$), $34813$ for ODR ($\beta=0.8$), and $30570$ for ODR ($\beta=0.7$). The learning rate is $\eta_t = \frac{\sqrt{M}}{10^5\sqrt{t}}$. The maximal number of iterations, i.e., $T$, is set to be $5\times 10^5$.  
