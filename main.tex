...
\begin{align}
\nonumber
\Rcal_T = & \sum_{i=1}^n\sum_{t=1}^T f_{i,t}(\x_{i,t}) - f_t(\x_t^\ast).
\end{align} The budget of the dynamics is defined as
\begin{align}
\nonumber
...
\le & TGn \eta + \frac{Gn}{2(1-\rho)^2} \sum_{t=1}^T \eta  + \frac{2\sqrt{R}n}{\eta}D + \frac{Rn}{2\eta}.
\end{align}
...
=& \frac{1}{n}\sum_{i=1}^n \beta \lrincir{\bar{f}_{i,t}(\x_{i,t}) - f_t(\x_t^\ast)} + (1-\beta) \EE \lrincir{f(\x_{i,t};\xi_i) - f(\x_t^\ast)} \\ \nonumber
