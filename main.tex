...
\end{enumerate} Thus, we let $f_{i,t}(\x;\xi_{i,t}) = \log\lrincir{1+\exp(-\y_{i,t}\A_{i,t}\Tr \x)} + \frac{\gamma}{2}\lrnorm{\x}^2$, where $\gamma = 10^{-3}$ is the given hyper-parameter.   When $M>0$, the learning rate $\eta_t$ is set to be $\frac{\sqrt{M}}{10^4\sqrt{t}}$ at the $t$-th iteration.
