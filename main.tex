...
\end{enumerate}    Then, we conduct decentralized online logistic regression by using the synthetic data.  The learning rate is $\eta_t = \frac{1}{\sqrt{t}}$. At every iteration $t$, the reference point $\x_t^\ast$ is approximately computed by performing  gradient descent $5$ steps. The maximal number of iterations, i.e., $T$, is set to be $5\times 10^5$.  
