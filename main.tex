...
\paragraph{Real data: xxx}

\paragraph{Real data: yyy}

\paragraph{Real data: zzz}


\subsection{DOG is as effective as COG in terms of regret}

\subsection{DOG is robust to the network topology}

\subsection{DOG is robust to the network size} 
...
First, we simulate a decentralized network consisting of $100$ nodes. Those nodes are connected by using a ring topology. We test the averaged loss and the averaged cumulative loss for every node to compare the performance of DOG and COG.  As shown in Figure \ref{figure_synthetic_loss}, both DOG and COG is effective to optimize the decision variable, and they have very similar performance. 

\begin{figure*}[!h]
\setlength{\abovecaptionskip}{0pt}
\setlength{\belowcaptionskip}{0pt}
\centering 
\subfigure[\textit{Number of iterations}]{\includegraphics[width=0.32\columnwidth]{figure_decen_cen_ave_loss_iterations}\label{figure_decen_cen_ave_loss_iterations}}
\subfigure[\textit{Network size}]{\includegraphics[width=0.32\columnwidth]{figure_decen_cen_ave_loss_nodes}\label{figure_decen_cen_ave_loss_nodes}}
\subfigure[\textit{Network topology}]{\includegraphics[width=0.32\columnwidth]{figure_decen_cen_ave_loss_iterations_net_types}\label{figure_decen_cen_ave_loss_iterations_net_types}}
\caption{The performance of DOG is comparable to that of COG.}
\label{figure_synthetic_loss}
\end{figure*}


Second, we evaluate the performance of DOG by varying the network size. As shown in Figure \ref{figure_decen_cen_ave_cumu_loss_nodes}, the averaged cumulative loss decreases effectively for a large network. After that, we change the topology of the network, and still evaluate the performance of DOG. The topology  \textit{Fully connected} means all nodes are connected, where DOG de-generates to be COG. The topology. The topology  \textit{WattsStrogatz} represents a Watts-Strogatz small-world graph. There is a parameter can be tuned, e.g., $0.5$ or $1$ in legend of Figures \ref{figure_decen_cen_ave_loss_iterations_net_types} and \ref{figure_decen_cen_cumu_loss_iterations_net_type}, to control the number of random edges in the Watts-Strogatz small-world graph.  As illustrated in Figures \ref{figure_decen_cen_ave_loss_iterations_net_types} and \ref{figure_decen_cen_cumu_loss_iterations_net_type}, the topology  \textit{Fully connected} has the best performance. But, there is no significant difference between all topologies.
