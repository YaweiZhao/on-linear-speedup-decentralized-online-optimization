...
We let $g_{i,t}(\x) = \log\lrincir{1+\exp(-\y_{i,t}\A_{i,t}\Tr \x)} + \frac{\gamma}{2}\lrnorm{\x}$ and $h_t(\x;\xi_{i,t}) = \log\lrincir{1+\exp(-\y_{i,t}\Xi_{i,t}\Tr \x)} + \frac{\gamma}{2}\lrnorm{\x}$, where $\gamma = 10^{-3}$ is the given hyper-parameter. $\A_{i,t}$ is the instance for the $i$-th node at the $t$-th iteration. $\y_{i,t}\in\{1,-1\}$ is the label of $\A_{i,t}$. At the $t$-iteration, $\Xi_{i,t}$ is sampled from a distribution $D_t$. The dimension of every instance is $d = 10$, that is, $\A_{i,t}\in\RR^{10}$. We generate the synthetic data as follows.
\begin{enumerate}
