...
We simulate a decentralized network consisting of $5$ nodes. Those nodes are connected by using a ring topology. Besides, we conduct online logistic regression by using datasets: \textit{room-occupancy}\footnote{\url{https://archive.ics.uci.edu/ml/datasets/Occupancy+Detection+}},  in the decentralized network. All values of a feature have been normalized to be zero mean and one variance.  As we have shown in Section \ref{subsection_application_scenarions}, we test the dynamic regret in those three application scenarios.  
