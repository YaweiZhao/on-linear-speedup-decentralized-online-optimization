...
We let $g_{i,t}(\x) = \log\lrincir{1+\exp(-\y_{i,t}\A_{i,t}\Tr \x)} + \frac{\gamma}{2}\lrnorm{\x}$ and $h_t(\x;\xi_{i,t}) = \lrangle{\x, \xi_{i,t}}$, where $\gamma = 0.1$ is the given hyper-parameter. $\A_{i,t}$ is the instance for the $i$-th node at the $t$-th iteration. $\y_{i,t}\in\{1,-1\}$ is the label of $\A_{i,t}$. The dimension of every instance is $d = 10$, that is, $\A_{i,t}\in\RR^{10}$. We generate the synthetic data as follows.
\begin{enumerate}
\item $\y_{i,t}$ is generated randomly. When $\y_{i,t}=1$, $\A_{i,t}$ is generated by sampling from a multi-variate Normal distribution $\A_{i,t} \sim N(\sin(t)\cdot\1, \I)$. When $\y_{i,t} = -1$, $\A_{i,t}$ is generated by sampling from another multi-variate Normal distribution $\A_{i,t} \sim N(-\sin(t)\cdot\1, \I)$.
\item $\xi_{i,t}$ is drawn from a time-varying Normal distribution, that is, $\xi_{i,t} \sim N(\0, \frac{\cos(t)+1}{10}\cdot\I)$.
\end{enumerate}    

First, we evaluate the dyanmic regret by varying the budget of the dynamics $M$. 
...
First,  we evaluate the dynamic regret with a large budget of the dynamics. The dynamics budget $M$ is $4.5\times 10^4$.  At the $t$-th iteration, the reference point $\x_t^\ast$ is approximately computed by performing  gradient descent $5$ iterations.  The learning rate is $\eta_t = \frac{\sqrt{M}}{10^5\sqrt{t}}$. The maximal number of iterations, i.e., $T$, is set to be $5\times 10^5$.  

\begin{figure*}[!h]
\setlength{\abovecaptionskip}{0pt}
\setlength{\belowcaptionskip}{0pt}
\centering 
\subfigure[\textit{Regret v.s. time}]{\includegraphics[width=0.45\columnwidth]{figure_synthetic_regret_times}\label{figure_synthetic_regret_times}}
\subfigure[\textit{Regret v.s. network size}]{\includegraphics[width=0.45\columnwidth]{figure_synthetic_regret_nodes}\label{figure_synthetic_regret_nodes}}
\caption{Comparsion of the dynamic regret by using  online logistic regression in the decentralized network.}
