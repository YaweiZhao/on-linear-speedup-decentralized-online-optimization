...
\item ({\bf Improve existing static regret in the decentralized setting \citep{Wenpeng's paper}})
\end{itemize} 
...
\end{align}
\end{Theorem}
The result suggests that $\x_{i,t}$ approaches to $\bar{\x}_t$ roughly in the rate $O(1/{T})$, which is faster than the convergence of the averaged regret, which is roughly in the rate $O(1/\sqrt{T})$ from Corollary~\ref{XXXX}. 
{\rc discuss the regret if using Wenpeng's or Alan Qi's paper definition for regret by using Theorem 2 and Theorem 1's result. We can even provide an upper bound on that.}
