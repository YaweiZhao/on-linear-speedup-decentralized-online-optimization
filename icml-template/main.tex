...
\end{itemize}
\item ({\bf Insight}) Consider the baseline that all users do not communicate but only run local online gradient. It is not hard to see that the static regret for this baseline approach is $O(n\sqrt{T}G + n\sqrt{T}\sigma)$.
Recall that $G$ measures the magnitude of the adversarial component and $\sigma$ measures the random component. 
