...
We make the following assumptions to analyze the dynamic regret theoretically.
\begin{Assumption}
...
\end{align} with $0<\beta<1$, and $\xi_{i,t}$ is a random variable drawn from an unknown distribution $D_t$. Note that $g_{i,t}$ is an adversary loss function, which is yielded by the learning model. $h_t(\cdot; \xi_{i,t})$ is a stochastic loss function, which depends on the random variable $\xi_{i,t}$. The expectation of $h_t(\cdot; \xi_{i,t})$ is a global model, and does not depend on the $i$-th node. 
...
Corollary \ref{corollary_regret_upper_bound} shows that DOG achieves sublinear regret.  The regret becomes large with increase of the budget of dynamics $M$. When $n=1$ and $\rho =0$, Corollary \ref{corollary_regret_upper_bound} shows that the dynamic regret is $\Ocal{\sqrt{TM}+\sqrt{T}}$, which is tight in the case of $n=1$ \citep{Zhao:2018wx}. When $\beta<1$, the second item of \eqref{equa_result_corollary} depends on the variance $\sigma^2$, instead of $n\sigma^2$. It benefits from the communication among nodes in the decentralized setting. Since every node shares its decision variable with its neighbours, the variance of the average of stochastic gradients $\frac{1}{n}\sum_{i=1}^n\nabla h_t(\x_{i,t};\xi_{i,t})$ is decreased to be $\frac{\sigma^2}{n}$, thus eventually reducing the regret caused by the stochastic data.   Additionally, Corollary \ref{corollary_regret_upper_bound} shows that the regret is impacted by the topology of the network, which is measured by $\rho$ with $0\le \rho < 1$. For a fully connected network\footnote{When a network is fully connected, a decentralized method de-generates to a centralized method.}, $\rho = 0$, and regret in the topologies is better than that in other topologies. 

\subsection{Connections with previous work}
Using the classic dynamic regret defined in \eqref{equa_definition_previous_regret}, \citet{8015179Shahram} has provided a $\Ocal{n\sqrt{nTM}}$ regret for DOG.  When $\beta = 1$, our new regret de-generates to the classic regret. Comparing with the result in \citet{8015179Shahram}, Corollary \ref{corollary_regret_upper_bound} achieves the state-of-the-art dependence on $T$ and $M$, and meanwhile improves the dependence on $n$. The improvement is caused by a better bound on the difference between $\x_{i,t}$ and $\bar{\x}_t$\footnote{\citet{8015179Shahram} denotes $\lrnorm{\x_{i,t} - \bar{\x}_t}$ by ``network error".}.
\begin{Lemma}
