...
& \Rcal_T^{\textsc{DOG}} \\ \nonumber
\lesssim & n \sqrt{T\lrincir{M+\sqrt{R}}G}  + \sqrt{n T\lrincir{M+\sqrt{R}}}\sigma \\ \nonumber
& + \frac{n\lrincir{M+\sqrt{R}}}{1-\rho} + \sqrt{\frac{T(M+\sqrt{R})\lrincir{n^2G + n \sigma^2}}{1-\rho}} 
\end{align}
\end{Corollary}
For simpler discussion, let us treat $R$, $G$, and $1-\rho$ as constants. The dynamic regret can simplified into $O(n\sqrt{MT}G + \sqrt{nMT}\sigma)$. The discussion for the dynamic regret is conducted in the following
\begin{itemize}
\item ({\bf Insight}) Recall that $G$ measures the magnitude of the adversarial component and $\sigma$ measures the random component. 
\item ({\bf Tightness})
\item ({\bf Comparison to \citet{}})
\item ({\bf Comparison to \citet{}})
\end{itemize} 
