...
for i=1:nn/n
    for j=1:n
        ...
        Xi(:,(i-1)*n+j) = cos(i)/10 +1+rand(d,1);
    end
...
for t=1:T
    ...
    Loss_basic_lr(:,1) = Loss_basic_lr(:,1) + log(1 + exp(-y((t-1)*n+1,:)*transpose(A(:,(t-1)*n+1))...
        * X_t_basic_lr(:,1))) + gamma/2*(transpose(X_t_basic_lr(:,1))*X_t_basic_lr(:,1));
    Loss_our_lr1(:,1) = Loss_our_lr1(:,1) + beta1 * (log(1 + exp(-y((t-1)*n+1,:)*transpose(A(:,(t-1)*n+1))...
        *X_t_our_lr1(:,1))) + gamma/2*(transpose(X_t_basic_lr(:,1))*X_t_basic_lr(:,1))) + (1-beta1)...
        * (transpose(Xi(:,(t-1)*n+1))*X_t_our_lr1(:,1));
    Loss_our_lr2(:,1) = Loss_our_lr2(:,1) + beta2 * (log(1 + exp(-y((t-1)*n+1,:)*transpose(A(:,(t-1)*n+1))...
        *X_t_our_lr2(:,1)))+ gamma/2*(transpose(X_t_basic_lr(:,1))*X_t_basic_lr(:,1))) + (1-beta2)...
        * (transpose(Xi(:,(t-1)*n+1))*X_t_our_lr2(:,1))+ gamma/2*(transpose(X_t_basic_lr(:,1))*X_t_basic_lr(:,1));
    Loss_our_lr3(:,1) = Loss_our_lr3(:,1) + beta3 * (log(1 + exp(-y((t-1)*n+1,:)*transpose(A(:,(t-1)*n+1))...
        *X_t_our_lr3(:,1)))+ gamma/2*(transpose(X_t_basic_lr(:,1))*X_t_basic_lr(:,1))) + (1-beta3)...
    ...
    if t==T
        ...
        cumu_obj_basic_lr = log(1 + exp(ones(1,d) * (transpose(-y(1:n:(t-1)*n+1,:)*ones(1,d))...
            ...
            +gamma/2*(norm(x_ast_basic,'fro')*norm(x_ast_basic,'fro'));
        ...
        cumu_obj_our_lr1 =  beta1 * log(1 + exp(ones(1,d) *( transpose(-y(1:n:(t-1)*n+1,:)*ones(1,d))...
            ...
            +(1-beta1) * ones(1,d)*(Xi(:,1:n:(t-1)*n+1) .* x_ast1)* ones(t,1)...
            +gamma/2*(norm(x_ast1,'fro')*norm(x_ast1,'fro'));
        ...
        cumu_obj_our_lr2 =  beta2 * (log(1 + exp(ones(1,d)*( transpose(-y(1:n:(t-1)*n+1,:)*ones(1,d))...
            ...
            +gamma/2*(norm(x_ast2,'fro')*norm(x_ast2,'fro')))...
            +(1-beta2) * ones(1,d)*(Xi(:,1:n:(t-1)*n+1) .* x_ast2)* ones(t,1);
        ...
        cumu_obj_our_lr3 =  beta3 * (log(1 + exp(ones(1,d)*( transpose(-y(1:n:(t-1)*n+1,:)*ones(1,d))...
            .* A(:,1:n:(t-1)*n+1) .* x_ast2)))*ones(t,1)...
            +gamma/2*(norm(x_ast2,'fro')*norm(x_ast2,'fro')))...
            +(1-beta3) * ones(1,d)*(Xi(:,1:n:(t-1)*n+1) .* x_ast2)* ones(t,1);
