...
\author{
  $^{1,2}$Yawei Zhao\footnote{Equal contribution.}, $^2$Chen Yu$^\ast$, $^{3}$Peilin Zhao, and $^{2}$Ji Liu \\
  % School of Computer$^1$\\
  $^1$National University of Defense Technology, Changsha, China\\
  $^2$University of Rochester, Rochester, USA \\
  $^3$ Tencent AI Lab, Shenzhen, China\\
  \texttt{zhaoyawei@nudt.edu.cn}, 
  \texttt{yuchen92328@gmail.com}, \\\texttt{peilinzhao@hotmail.com}, \texttt{ji.liu.uwisc@gmail.com} \\
}
...
\begin{align}
\nonumber
\Xi_{n,T} =  \{ \xi_{i,t} \}_{1\le i \le n, 1 \le t \le T}, {~~~~}\Dcal_{n,T} =  \{ D_{i,t} \}_{1\le i \le n, 1 \le t \le T},
...
\widetilde{\Rcal}_T^{A} :=  \EE_{ \Xi_{n,T} \sim \Dcal_{n,T} }  \left[\sum_{i=1}^n\sum_{t=1}^T \lrincir{f_{i,t}(\x_{i,t};\xi_{i,t}) - f_{i,t}(\x^\ast;\xi_{i,t}} \right],
\end{align} 
...
\Rcal_T^{A} :=  \EE_{ \Xi_{n,T} \sim \Dcal_{n,T} }  \left[\sum_{i=1}^n\sum_{t=1}^T f_{i,t}(\x_{i,t};\xi_{i,t}) \right] - \min_{\{\x_t^\star\}_{t=1}^T \in \mathcal{L}_{M}^T}  \EE_{ \Xi_{n,T} \sim \Dcal_{n,T} }\left[\sum_{i=1}^n\sum_{t=1}^T f_{i,t}(\x_t^\ast;\xi_{i,t}) \right],
\end{align}
  ...
  \State Initialize $\x_{i,1} = \0$ for all $i\in [n]$;    
   \For {$t=1,2, ..., T$}
            ...
            % \State \indent Predict $\x_{i,t}$.
                        \State Query the neighbors' local models $\{\x_{j,t}\}_{j\in \text{user $i$'s neighbor set}}$;
            \State Observe the loss function $f_{i,t}$, and suffer loss $f_{i,t}(\x_{i,t};\xi_{i,t})$;
            %\State \textbf{Update:}
            \State Query the gradient $\nabla f_{i,t}(\x_{i,t};\xi_{i,t})$;
            \State Update the local model by 
...
\begin{align}
\nonumber
\EE_{ \Xi_{n,T} \sim \Dcal_{n,T} } \sum_{t=1}^T\sum_{i=1}^n f_{i,t}(\x_{i,t};\xi_{i,t}) - f_{i,t}(\x_t^\ast;\xi_{i,t}) \le  20\eta T n G^2 +  \eta T\sigma^2 + C nT\eta^2 G^2    + \frac{n}{2\eta}\lrincir{ 4\sqrt{R}M + R  }.
\end{align}
\end{Theorem}
...
\Rcal_T^{\textsc{DOG}} \lesssim  \frac{n\lrincir{M+\sqrt{R}}}{1-\rho} + \sqrt{\frac{T\lrincir{M+\sqrt{R}}(n^2G^2 + n\sigma^2)}{1-\rho}}.
\end{align}
\end{Corollary}
...
\widehat{\Rcal}_T^{A}(\x_{j,t}) := \EE_{\Xi_{n,T} \sim \Dcal_{n,T}}\left [\sum_{i=1}^n \sum_{t=1}^T f_{i,t}(\x_{j,t};\xi_{i,t})\right ] - \min_{\{\x_{i,t}^\ast\}_{t=1}^T \in \Lcal_M^T} \EE_{\Xi_{n,T} \sim \Dcal_{n,T}} \left [\sum_{i=1}^n \sum_{t=1}^T f_{i,t}(\x_{i,t}^\ast;\xi_{i,t})\right ],
\end{align} 
...
\begin{align}
\nonumber
\widehat{\Rcal}_T^{\textsc{DOG}}(\x_{j,t}) \lesssim & C_1 \eta n\sqrt{n} T G^2 +\eta T\sigma^2 + C_2  nT\eta^2 G^2    + \frac{n}{2\eta}\lrincir{ 4\sqrt{R}M + R  }.
\end{align}
...
\begin{align}
\nonumber
\widehat{\Rcal}_T^{\textsc{DOG}}(\x_{j,t}) \lesssim \sqrt{n}\lrincir{M+\sqrt{R}} + \frac{\sqrt{T\lrincir{M+\sqrt{R}}(n^\frac{5}{2}G^2 + n\sigma^2)}}{1-\rho}.
\end{align}
\end{Corollary} 
...
\centering 
\subfigure{\includegraphics[width=0.6\columnwidth]{figure_dynamics}\label{figure_dynamics}}
...
\setlength{\belowcaptionskip}{0pt}
\centering 
\subfigure[\textit{synthetic data}, $10000$ nodes,random topology]{\includegraphics[width=0.49\columnwidth]{figure_ave_loss_iteration_synthetic}\label{figure_ave_loss_iteration}}
\subfigure[\textit{room-occupancy}, $5$ nodes, ring topology]{\includegraphics[width=0.49\columnwidth]{figure_ave_loss_iteration_occupancy}\label{figure_ave_loss_iteration_occupancy}}
\subfigure[\textit{usenet2}, $5$ nodes, ring topology]{\includegraphics[width=0.49\columnwidth]{figure_ave_loss_iteration_usenet2}\label{figure_ave_loss_iteration_usenet2}}
\subfigure[\textit{spam}, $5$ nodes, ring topology]{\includegraphics[width=0.49\columnwidth]{figure_ave_loss_iteration_spam}\label{figure_ave_loss_iteration_spam}}
...
\begin{align}
\nonumber
& \EE_{ \Xi_{n,t} \sim \Dcal_{n,t} } \frac{1}{n}\sum_{i=1}^n \lrincir{ f_{i,t}(\x_{i,t};\xi_{i,t}) - f_{i,t}(\x_t^\ast;\xi_{i,t}) } \le  \EE_{ \Xi_{n,t} \sim \Dcal_{n,t} } \frac{1}{n}\sum_{i=1}^n \lrangle{ \nabla f_{i,t}(\x_{i,t};\xi_{i,t}),  \x_{i,t} - \x_t^\ast } \\ \nonumber
...
\begin{align}
\nonumber
\bar{\x}_{t+1} = \argmin_{\z\in\RR^d} \phi(\z) = \argmin_{\z\in\RR^d} \lrangle{\frac{1}{n}\sum_{i=1}^n \nabla f_{i,t}(\x_{i,t};\xi_{i,t}), \z} + \frac{1}{2\eta}\lrnorm{\z - \bar{\x}_t}^2.
...
\begin{align}
\nonumber
\EE_{ \Xi_{n,t} \sim \Dcal_{n,t} } \lrincir{F_{i,t}(\bar{\x}_t) - F_{i,t}(\bar{\x}_{t+1}) } \le & \EE_{ \Xi_{n,t} \sim \Dcal_{n,t} } \lrangle{\nabla F_{i,t}(\bar{\x}_t), \bar{\x}_t - \bar{\x}_{t+1}} \\ \nonumber
...
\begin{align}
\nonumber
\sum_{i=1}^n \lrnorm{\x_{i,t} - \bar{\x}_t}^2 \refabovecir{=}{\textcircled{1}} & \sum_{i=1}^n \lrnorm{ \sum_{s=1}^{t-1} \eta \bar{\G}_s - \eta \G_s \W^{t-s-1}\e_i }^2   \\ \nonumber
...
\begin{align}
\nonumber
\EE_{ \Xi_{n,T} \sim \Dcal_{n,T} } \sum_{i=1}^n\sum_{t=1}^T \lrnorm{\x_{i,t} - \bar{\x}_t}^2 \le & \EE_{ \Xi_{n,T} \sim \Dcal_{n,T} } \sum_{t=1}^T \lrincir{ \sum_{s=1}^{t-1} \eta \rho^{t-s-1} \lrnorm{\G_s}_F}^2  \\ \nonumber
